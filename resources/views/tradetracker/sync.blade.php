    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        @if(!$configured)
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded">
                TradeTracker is niet geconfigureerd. Voeg <code>TRADETRACKER_CUSTOMER_ID</code> en <code>TRADETRACKER_PASSPHRASE</code> toe aan je omgevingsvariabelen.
            </div>
        @elseif(!$revenueStream)
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded">
                Geen TradeTracker revenue stream gevonden. Maak er eerst een aan onder Inkomsten bronnen.
            </div>
        @else
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <p class="text-sm text-gray-500 mb-4">Haal transacties op voor één specifieke dag.</p>
                <form action="{{ route('tradetracker.sync.run') }}" method="POST" class="flex items-end gap-4">
                    @csrf
                    <div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <p class="text-sm text-gray-500 mb-4">
                    Haal transacties op voor een periode. Per dag wordt één importrecord aangemaakt.
                ...
                <div id="backfill-progress" class="hidden">
                    ...
                    <div id="unmatched-section" class="hidden">
                        <p class="text-sm font-medium text-orange-700 mb-2">Ongematchte transacties gevonden — klik op een datum om matchers in te stellen:</p>
        ...
        <!-- Dagen met ongematchte transacties -->
        @if($unmatchedDays->isNotEmpty())
        <div class="bg-orange-50 border border-orange-200 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h3 class="text-lg font-medium text-orange-700 mb-2">Ongematchte transacties — actie vereist</h3>
                <p class="text-sm text-orange-600 mb-4">
                    Voor de onderstaande dagen zijn transacties binnengekomen die niet automatisch gekoppeld konden worden.
                    Klik op een datum om matchers in te stellen.
                </p>
                <div class="flex flex-wrap gap-2">
                    @foreach($unmatchedDays as $date)
                        <form action="{{ route('tradetracker.fix-day') }}" method="POST">
                            @csrf
...
<form id="fix-day-form" action="{{ route('tradetracker.fix-day') }}" method="POST">
    @csrf
...
async function runDays(days) {
    ...
    for (let i = 0; i < days.length; i++) {
        ...
        try {
            const response = await fetch('{{ route('tradetracker.sync.day') }}', {
                method: 'POST',
                headers: {
